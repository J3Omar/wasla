...
class LinuxAudioService {
  ...
  Future<void> enableSystemAudioCapture() async {
    ...
    if (_nullSinkId != null) {
      debugPrint('[LinuxAudioService] Virtual sink already loaded with ID: $_nullSinkId');
      return;
    }

    debugPrint('[LinuxAudioService] Attempting to load virtual sink module-null-sink...');
    ...
    try {
      ...
      if (result.exitCode != 0 || result.stderr.toString().isNotEmpty) {
        debugPrint('[LinuxAudioService] Failed to load virtual sink. Exit Code: ${result.exitCode}');
      ...
      if (output.isNotEmpty) {
        _nullSinkId = output;
        debugPrint('[LinuxAudioService] Successfully loaded virtual sink. Module ID: $_nullSinkId');
      } else {
        debugPrint('[LinuxAudioService] Command succeeded but returned an empty Module ID.');
      }
    } catch (e) {
      debugPrint('[LinuxAudioService] Exception occurred while loading virtual sink: $e');
    }
  }
  ...
  Future<void> disableSystemAudioCapture() async {
    ...
    debugPrint('[LinuxAudioService] Attempting to unload virtual sink with Module ID: $_nullSinkId');
    ...
    try {
      ...
      if (result.exitCode != 0 || result.stderr.toString().isNotEmpty) {
        debugPrint('[LinuxAudioService] Failed to unload virtual sink. Exit Code: ${result.exitCode}');
        debugPrint('[LinuxAudioService] Stderr: ${result.stderr}');
      ...
      debugPrint('[LinuxAudioService] Successfully unloaded virtual sink Module ID: $_nullSinkId');
      _nullSinkId = null;
    } catch (e) {
      debugPrint('[LinuxAudioService] Exception occurred while unloading virtual sink: $e');
    }
  }
}
